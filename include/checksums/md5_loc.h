...
#define __MD5_LOC_H__
 ...
 /*
  * We don't include "conf.h" here because it gets included before this file in
  * md5.cpp so the defines are correctly determing before they are checked.
  */
#if MD5_DEBUG
#include <iostream>
#endif  // MD5_DEBUG
...
namespace md5 {
	 ...
	 */
	inline unsigned int cyclic_left_rotate(unsigned int data,
	...
	inline void FF(unsigned int& a, unsigned int b, unsigned int c, unsigned int d,
		unsigned int Xk, unsigned int s, unsigned int i) {
#if MD5_DEBUG
		std::cout << "\nA: " << a << "\nB: " << b << "\nC: " << c << "\nD: " << d
			<< "\nX[" << i << "]: " << Xk << "\ns: " << S1[s]
			<< "\nT: " << T[i] << "\n";
