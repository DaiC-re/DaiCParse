    ...
    EndToEndFixture() {
        ...
        if (std::filesystem::exists(test_binary_path)) {
            try {
                ...
                std::cerr << "Failed to load test binary for E2E: " << e.what()
                          << std::endl;
            }
        }
    }
...
TEST_CASE_METHOD(EndToEndFixture, "E2E: Complete Binary Analysis Pipeline",
                 "[E2E][Complete]") {
...
TEST_CASE_METHOD(EndToEndFixture, "E2E: Binary Parsing and Section Extraction",
                 "[E2E][Parsing]") {
    if (!isBinaryAvailable()) {
        SKIP("Test binary not available");
    }
    ...
    auto has_text = std::find(section_names.begin(), section_names.end(),
                              ".text") != section_names.end();
    REQUIRE(has_text);  // PE binaries should have .text
}

TEST_CASE_METHOD(EndToEndFixture, "E2E: Metadata Extraction and Validation",
                 "[E2E][Metadata]") {
    if (!isBinaryAvailable()) {
        SKIP("Test binary not available");
    }
...
TEST_CASE_METHOD(EndToEndFixture, "E2E: Section Navigation and RVA Lookup",
                 "[E2E][Navigation]") {
    if (!isBinaryAvailable()) {
        SKIP("Test binary not available");
    }
    ...
    try {
        ...
        if (text_section.virtual_size > 100) {
            auto mid_addr =
                text_section.virtual_addr + text_section.virtual_size / 2;
...
TEST_CASE_METHOD(EndToEndFixture, "E2E: Disassembly and Function Detection",
                 "[E2E][Disassembly]") {
    if (!isBinaryAvailable()) {
        SKIP("Test binary not available");
    }
...
TEST_CASE_METHOD(EndToEndFixture, "E2E: Section Content Iteration",
                 "[E2E][Iteration]") {
    if (!isBinaryAvailable()) {
        SKIP("Test binary not available");
    }
...
TEST_CASE_METHOD(EndToEndFixture, "E2E: File Checksum Calculation",
                 "[E2E][Checksums]") {
    if (!std::filesystem::exists(test_binary_path)) {
        SKIP("Test binary not available");
    }
...
TEST_CASE_METHOD(EndToEndFixture, "E2E: MD5 Hash Calculation",
                 "[E2E][Hashing]") {
    if (!std::filesystem::exists(test_binary_path)) {
        SKIP("Test binary not available");
    }
...
TEST_CASE_METHOD(EndToEndFixture, "E2E: Binary Hash Calculation",
                 "[E2E][BinaryHash]") {
    if (!isBinaryAvailable()) {
        SKIP("Test binary not available");
    }
    ...
    try {
        ...
        for (const auto& section : binary->sections) {
            ...
            hash_stream << "Section " << section.name << ": " << byte_count
                        << " bytes\n";
        }
...
TEST_CASE_METHOD(EndToEndFixture, "E2E: Hex Encoding of Section Data",
                 "[E2E][HexUtils]") {
    if (!isBinaryAvailable()) {
        SKIP("Test binary not available");
    }
...
TEST_CASE_METHOD(EndToEndFixture,
                 "E2E: Binary Section Serialization and Deserialization",
                 "[E2E][Serialization]") {
    if (!isBinaryAvailable()) {
        SKIP("Test binary not available");
    }
...
TEST_CASE_METHOD(EndToEndFixture, "E2E: Function Serialization Pipeline",
                 "[E2E][FunctionSerialization]") {
    if (!isBinaryAvailable()) {
        SKIP("Test binary not available");
    }
...
TEST_CASE_METHOD(EndToEndFixture, "E2E: Complete Analysis and Export Workflow",
                 "[E2E][Workflow]") {
    if (!isBinaryAvailable()) {
        SKIP("Test binary not available");
    }
    ...
    for (const auto& section : binary->sections) {
        ...
        export_stream << "  Virtual Addr: 0x" << std::hex
                      << section.virtual_addr << std::dec << "\n";
        export_stream << "  Virtual Size: 0x" << std::hex
                      << section.virtual_size << std::dec << "\n";
        export_stream << "  File Offset: 0x" << std::hex << section.offset
                      << std::dec << "\n";
    }
    ...
    for (const auto& func : binary->_functions) {
        export_stream << "  " << func.getName() << ": 0x" << std::hex
                      << func.getStart() << " - 0x" << func.getEnd() << std::dec
                      << "\n";
    }

    // Verify export was successful
    REQUIRE(export_stream.str().length() > 0);
    REQUIRE(export_stream.str().find("BINARY ANALYSIS REPORT") !=
            std::string::npos);
}

TEST_CASE_METHOD(EndToEndFixture, "E2E: Binary Comparison Workflow",
                 "[E2E][Comparison]") {
    if (!isBinaryAvailable()) {
        SKIP("Test binary not available");
    }
...
TEST_CASE_METHOD(EndToEndFixture, "E2E: Section Content Integrity Check",
                 "[E2E][Integrity]") {
    if (!isBinaryAvailable()) {
        SKIP("Test binary not available");
    }
...
TEST_CASE_METHOD(EndToEndFixture, "E2E: Large Content Iteration Performance",
                 "[E2E][Performance]") {
    if (!isBinaryAvailable()) {
        SKIP("Test binary not available");
    }
...
TEST_CASE_METHOD(EndToEndFixture, "E2E: Multiple Section Processing",
                 "[E2E][Stress]") {
    if (!isBinaryAvailable()) {
        SKIP("Test binary not available");
    }
...
TEST_CASE_METHOD(EndToEndFixture, "E2E: Graceful Handling of Missing Sections",
                 "[E2E][ErrorHandling]") {
    if (!isBinaryAvailable()) {
        SKIP("Test binary not available");
    }
...
TEST_CASE_METHOD(EndToEndFixture, "E2E: Handling Empty or Sparse Sections",
                 "[E2E][EdgeCases]") {
    if (!isBinaryAvailable()) {
        SKIP("Test binary not available");
    }
...
TEST_CASE_METHOD(EndToEndFixture, "E2E: Metadata Consistency Check",
                 "[E2E][Consistency]") {
    if (!isBinaryAvailable()) {
        SKIP("Test binary not available");
    }
...
TEST_CASE_METHOD(EndToEndFixture,
                 "E2E: Full Pipeline - Parse, Hash, Analyze, Export",
                 "[E2E][FullPipeline]") {
    if (!isBinaryAvailable()) {
        SKIP("Test binary not available");
    }
...
TEST_CASE_METHOD(EndToEndFixture,
                 "E2E: Round-trip Test (Parse -> Serialize -> Re-Parse)",
                 "[E2E][RoundTrip]") {
    if (!isBinaryAvailable()) {
        SKIP("Test binary not available");
    }
