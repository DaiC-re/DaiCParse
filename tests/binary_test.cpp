    ...
    BinaryFixture() {
        ...
        if (std::filesystem::exists(test_binary_path)) {
            try {
                ...
                std::cerr << "Failed to load test binary: " << e.what()
                          << std::endl;
            }
        }
    }
...
TEST_CASE("Binary::Function Multiple Serialization Round-Trips",
          "[Binary][Function]") {
...
TEST_CASE("Binary section_from_rva - Single Section", "[Binary][Sections]") {
    ...
    sections.push_back(
        BinSection(".text", content, {}, 0x1000, 0x4000, 0x1000));

    // Test RVA lookup logic
    auto addr = 0x4000UL + 0x100;  // Within section
    auto result = std::find_if(
        sections.begin(), sections.end(), [addr](const BinSection& section) {
...
TEST_CASE("Binary section_from_rva - Multiple Sections", "[Binary][Sections]") {
    ...
    sections.push_back(
        BinSection(".text", content, {}, 0x1000, 0x4000, 0x1000));
    sections.push_back(
        BinSection(".data", content, {}, 0x2000, 0x5000, 0x1000));
    sections.push_back(
        BinSection(".rsrc", content, {}, 0x3000, 0x6000, 0x1000));
...
TEST_CASE("Binary section_from_rva - Boundary Conditions",
          "[Binary][Sections]") {
    std::vector<BinSection> sections;
    std::vector<uint8_t> content_data = {0x01, 0x02};
    LIEF::span<const uint8_t> content(content_data.data(), content_data.size());
...
TEST_CASE("Binary Function Storage and Retrieval", "[Binary][Functions]") {
    ...
    auto func_it = std::find_if(
        ...
        [](const Binary::Function& func) { return func.getStart() == 0x3000; });
...
TEST_CASE_METHOD(BinaryFixture, "Binary loads dummy test binary",
                 "[Binary][Integration]") {
...
TEST_CASE_METHOD(BinaryFixture, "Binary has sections after loading",
                 "[Binary][Integration]") {
...
TEST_CASE_METHOD(BinaryFixture, "Binary metadata is initialized",
                 "[Binary][Integration]") {
    if (!std::filesystem::exists(test_binary_path) || !binary) {
        SKIP("Test binary not available");
    }
...
TEST_CASE_METHOD(BinaryFixture, "Binary can find .text section",
                 "[Binary][Integration]") {
    if (!std::filesystem::exists(test_binary_path) || !binary) {
        SKIP("Test binary not available");
    }
...
TEST_CASE_METHOD(BinaryFixture, "Binary instruction count is positive",
                 "[Binary][Integration]") {
    if (!std::filesystem::exists(test_binary_path) || !binary) {
        SKIP("Test binary not available");
    }
...
TEST_CASE_METHOD(BinaryFixture, "Binary text section virtual address is valid",
                 "[Binary][Integration]") {
    if (!std::filesystem::exists(test_binary_path) || !binary) {
        SKIP("Test binary not available");
    }
...
TEST_CASE_METHOD(BinaryFixture, "Binary section lookup by RVA works",
                 "[Binary][Integration]") {
    if (!std::filesystem::exists(test_binary_path) || !binary) {
        SKIP("Test binary not available");
    }
...
TEST_CASE_METHOD(BinaryFixture,
                 "Binary section lookup returns nullptr for invalid RVA",
                 "[Binary][Integration]") {
    if (!std::filesystem::exists(test_binary_path) || !binary) {
        SKIP("Test binary not available");
    }
...
TEST_CASE_METHOD(BinaryFixture, "Binary sections have valid properties",
                 "[Binary][Integration]") {
    if (!std::filesystem::exists(test_binary_path) || !binary) {
        SKIP("Test binary not available");
    }
...
TEST_CASE_METHOD(BinaryFixture, "Binary has no overlapping sections",
                 "[Binary][Integration]") {
    if (!std::filesystem::exists(test_binary_path) || !binary) {
        SKIP("Test binary not available");
    }
    ...
    for (size_t i = 0; i < binary->sections.size(); ++i) {
        for (size_t j = i + 1; j < binary->sections.size(); ++j) {
            ...
            // Check for overlap
            bool overlap = !(sec1_end <= sec2.virtual_addr ||
                             sec2_end <= sec1.virtual_addr);
            REQUIRE(!overlap);
        }
    }
}

TEST_CASE_METHOD(BinaryFixture, "Binary functions can be created and retrieved",
                 "[Binary][Integration]") {
    if (!std::filesystem::exists(test_binary_path) || !binary) {
        SKIP("Test binary not available");
    }
