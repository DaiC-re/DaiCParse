...
void Binary::detectFunctions() {
    ...
    if (count > 0) {
        ...
        for (j = 0; j < count; j++) {
            ...
            // Function qui chercherait � d�tecter tout les calls et qui sauvegarde l'adresse de destination dans un vecteur,
            // une fois qu'on a ce vecteur on peut commencer detectFunctions classique, si on est � une adresse qui est dans
            // le vecteur, on met current_function_start � cette adresse
