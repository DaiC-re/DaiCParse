...
namespace md5 {
	 ...
	 */
	md5_t::md5_t(const void* input, const unsigned int input_length,
		void* signature) {
	...
	void md5_t::process(const void* input, const unsigned int input_length) {
		if (!finished) {
			...
			/*
			 * If we have any data stored from a previous call to process then we
			 * use these bytes first, and the new data is large enough to create a
			 * complete block then we process these bytes first.
			 */
			...
			/*
			 * If there are any unprocessed bytes left over that do not create a
			 * complete block then we store these bytes for processing next time.
			 */
			if (processed != input_length) {
				memcpy(stored + stored_size, (char*)input + processed,
					input_length - processed);
	...
	void md5_t::finish(void* signature_) {
		if (!finished) {
			...
			unsigned int size_high =
				(message_length[1] << 3) | ((message_length[0] & 0xE0000000) >> 29);
	...
	void md5_t::initialise() {
		/*
		 * ensures that unsigned int is 4 bytes on this platform, will need
		 * modifying if we are to use on a different sized platform.
		 */
		assert(MD5_SIZE == 16);
	...
	void sig_to_string(const void* signature_, char* str_, const int str_len) {
		...
		for (sig_p = (unsigned char*)signature_;
			sig_p < (unsigned char*)signature_ + MD5_SIZE; sig_p++) {
			high = *sig_p / 16;
			low = *sig_p % 16;
			/* account for 2 chars */
			if (str_p + 1 >= max_p) {
