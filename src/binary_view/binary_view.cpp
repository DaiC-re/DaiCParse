...
BinaryView::BinaryView(Binary* binary) : _binary(binary) {
    _relative_text_addr = _binary->getTextSectionVirtualAddr();
}

BinSection* BinaryView::getSectionAtAddr(uintptr_t virtual_addr) {
    auto& sections = _binary->sections;

    std::cout << std::format("Looking for section at addr: {:#x}\n",
                             virtual_addr);

    BinSection* closest = nullptr;
    uintptr_t closest_addr = 0;

    for (auto& section : sections) {
        auto sec_addr = section.virtual_addr;
...
BinSection* BinaryView::getNextSection(BinSection* section) {
    auto& sections = _binary->sections;
    auto section_it =
        std::find_if(sections.begin(), sections.end(),
                     [section](BinSection& sec) { return &sec == section; });
    ++section_it;
...
std::vector<uint8_t> BinaryView::getSectionContentFromAddr(
    size_t addr, size_t content_size = CHUNK_SIZE) {
    BinSection* section = _binary->section_from_rva(addr);
    std::cout << std::format("addr: {:#x}, section: {}\n", addr,
                             section != nullptr ? section->name : "nullptr");
    if (section == nullptr) {
        std::cerr << "No section found at this address, dev error\n";
        section = static_cast<BinSection*>(getSectionAtAddr(addr));
    }
    std::array<LIEF::span<const uint8_t>, 2> contents = {section->content,
                                                         section->padding};
    auto start_offset = addr - section->virtual_addr;
    std::cout << std::format(
        "addr: {:#x}, section addr: {:#x}, start_offset: {:#x}\n", addr,
        section->virtual_addr, start_offset);
    auto range = contents | std::views::join | std::views::drop(start_offset) |
                 std::views::take(content_size);
    ...
    if (vector.size() != content_size) {
        ...
        if (addr + content_size >= next_section->virtual_addr) {
            auto second_vector = getSectionContentFromAddr(
                next_section->virtual_addr, content_size - vector.size());
            vector.insert(vector.end(), second_vector.begin(),
                          second_vector.end());
        }
        std::fill(vector.begin() + vector.size(), vector.begin() + content_size,
                  0);
    }
    return vector;
}
...
std::string BinaryView::viewDisasmChunk(size_t index) {
    ...
    auto content =
        getSectionContentFromAddr(instruction_addr, size_between_checkpoints);
    return _binary->contentToDisasm(instruction_addr, content);
}

std::pair<uintptr_t, CheckPoint> BinaryView::getDisasmInstructionAddr(
    size_t index) {
    ...
    auto text_content = getSectionContentFromAddr(
        check_point_addr, 0x15 * _binary->_instructions_per_checkpoint);
    auto it = text_content.begin();
    auto subrange_from_checkpoint =
        std::ranges::subrange(it, text_content.end());

    auto instruction_addr = _binary->getTargetFromCheckpoint(
        {check_point_index, check_point_addr}, index, subrange_from_checkpoint);

    return {instruction_addr, CheckPoint{check_point_index, check_point_addr}};
}
